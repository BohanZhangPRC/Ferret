...
def get_sessions(sheet_name, sheet_id, headstage_of_interest = 0, session_filter=None, mounted = 'auto'):
...
def load_sessions(sheet_ids, headstage_of_interest, session_type):
    """"
...
def get_data_spike_sorted(all_sessions_path, dt,  gc_or_not = False, remove_baseline = False) :
...
def get_data(all_sessions_path, dt, gc_or_not=True):
    ### version qui gère le tracking only s'il n'y a pas de mock. 
    ...
    for file in tqdm(all_sessions_path):
        ...
        try:
            ...
            data_file = os.path.join(file, f'data_{dt}.npy')
            feat_file = os.path.join(file, f'features_{dt}.npy')

            if not os.path.exists(data_file) or not os.path.exists(feat_file):
                print(f"Missing data/features in {file}, skipping")
                continue

            # ---------- Chargement ----------
            n_data = np.load(data_file)
            f_data_raw = np.load(feat_file, allow_pickle=True)
            unique_tones = np.load(os.path.join(file, 'unique_tones.npy'))

            # ---------- good_clusters sécurisé ----------
            gc_file = os.path.join(file, 'good_clusters.npy')
            if gc_or_not and os.path.exists(gc_file):
                gc = np.load(gc_file)
            else:
                gc = np.arange(n_data.shape[0])
            ...
            if 'Played_frequency' not in f_data.columns:
                raise ValueError("Played_frequency missing")
            ...
            # ---------- Directions ----------
            f_data['Change_direction'] = compare_diff(
                f_data['Played_frequency'].to_numpy()
            )

            if 'Mock_frequency' in f_data.columns:
                f_data['Mock_direction'] = compare_diff(
                    f_data['Mock_frequency'].to_numpy()
                )
            else:
                f_data['Mock_direction'] = np.nan

            # ---------- Mapping fréquence -> position ----------
            unique_tones_sorted = np.sort(unique_tones)
            ...
            if 'Mock_frequency' in f_data.columns:
                ...
                positional_freq = f_data['Played_frequency']
            ...
            # ---------- Speed ----------
            speed_x = np.diff(pos_smooth, prepend=pos_smooth[0])
            speed_x = np.abs(speed_x) * 100
            ...
            sound_speed = np.diff(freq_pos_smooth, prepend=freq_pos_smooth[0])
            sound_speed = np.abs(sound_speed) * 100
            sound_speed[~np.isfinite(sound_speed)] = 0
...
def get_data_old(all_sessions_path, dt, gc_or_not = True) :
...
def re_organise_data(n_data_all, f_data_all):
    ...
    for i in range(n_sessions):
        for j in range(len(n_data_all)):

            n_data = n_data_all[j][i]
            f_data = f_data_all[j][i]

            # skip si vide
            if n_data is None or f_data is None:
                continue
            if len(n_data) == 0 or len(f_data) == 0:
                continue

            n_data_reorganised.append(n_data)
            f_data_reorganised.append(f_data)
