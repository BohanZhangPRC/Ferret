...
def train_one_session(model, n_data_session, f_df, session_idx,
    ...
    for val, label in [(0.0, "Tracking"), (1.0, "Playback")]:
        ...
        for dk in DRIVE_KEYS:
            _, dl, _ = extract_epochs(n_data_session, f_df, val, dt, label_col=dk)
            drive_per_key[dk] = dl
        # Sanity checks: same epoch count and lengths as CEBRA extraction
        for dk in DRIVE_KEYS:
    ...
    # Within-epoch split uses first 80% of time points for training,
    # last 20% for validation — works even with a single epoch.
    ...
    for cond_name in ["Tracking", "Playback"]:
        ...
        if n_ep >= 2:
            ...
            val_epochs[cond_name] = {
                'n': cd['n'][n_train:], 'l': cd['l'][n_train:],
                'd': cd['d'][n_train:],
            }
        else:
            # Within-epoch split: train on first 80%, validate on last 20%
            for ep_n, ep_l, ep_d in zip(cd['n'], cd['l'], cd['d']):
                T_ep = len(ep_n)
                split = int(T_ep * TRAIN_VAL_SPLIT)
                if split < MINI_TRAJ_LEN or (T_ep - split) < MINI_TRAJ_LEN:
                    # Epoch too short even for within-epoch split — use all for train
                    all_train_n.append(ep_n)
                    all_train_l.append(ep_l)
                    all_train_d.append(ep_d)
                    val_epochs[cond_name] = {'n': [], 'l': [], 'd': []}
                else:
                    all_train_n.append(ep_n[:split])
                    all_train_l.append(ep_l[:split])
                    all_train_d.append(ep_d[:split])
                    val_epochs[cond_name] = {
                        'n': [ep_n[split:]], 'l': [ep_l[split:]],
                        'd': [ep_d[split:]],
                    }

    if not all_train_n:
        print(f"  Session {session_idx}: no training data, skipping")
        return model, {}, {}
