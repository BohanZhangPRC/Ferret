...
if ablation_df is not None:
    ...
    if len(session_avg) > 1:
        ...
        print(f"  Paired R2_drive (E2E vs OLS_λ=0, per-session avg): "
              f"t={t_a:.3f}, p={p_a:.4f}")
        if p_a < 0.05 and t_a > 0:
            print(f"  -> Dynamics constraint significantly IMPROVES R2_drive over")
            print(f"     InfoNCE-only embedding (E2E > OLS_λ=0).")
        elif p_a < 0.05 and t_a < 0:
            print(f"  -> Dynamics constraint significantly DEGRADES R2_drive vs.")
            print(f"     InfoNCE-only embedding (E2E < OLS_λ=0). The jointly-trained")
            print(f"     encoder may be forced into a worse configuration than pure")
            print(f"     InfoNCE would produce — the rotational dynamics are actively")
            print(f"     harmful for trajectory prediction at this horizon.")
        else:
            print(f"  -> No significant difference (t={t_a:.3f}, p={p_a:.4f}): InfoNCE alone")
            print(f"     produces comparable rotational structure. The dynamics")
            print(f"     constraint adds little.")
